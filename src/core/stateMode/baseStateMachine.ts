 ...
 * @LastEditTime : 2021-07-23 16:21:02
...
export class BaseStateMachine {
	...
	constructor(
		...
		console.log(this.instanceExecutionQueue, 'init')
	}
	/**
	 * @description 按需初始化
	 */
	protected onDemandInitialization() {
		this.stateInstance.forEach((instance, instanceIndex) => this.#waitingForInstance({ fn: instance, params: this.executionQueueParameters[instanceIndex], initialization: false }))
	}
	/**
	 * @description 立即初始化
	 */
	 ...
	 * @return { void }
	 */
	#instantiatedState(instance: Function & dispatchAbstract, params: ArrayLike<any>): void {
		hasProperty(
			instance,
			CURRENT_STATE,
			() => {
				warn(
					`${instance.name
					} Please check if the function has ${CURRENT_STATE} static property!`
				);
			},
			() => {
	 ...
	 * @return { void }
	 */
	#waitingForInstance(instance: { fn: Function & dispatchAbstract, params: ArrayLike<any>, initialization: boolean }): void {
		// @ts-ignore
		hasProperty(
			instance.fn,
			CURRENT_STATE,
